...
import { Send, RefreshCw, AlertTriangle, ArrowUpRight, FileText, Clock, ChevronRight, CheckCircle } from 'lucide-react';
              ...
              <tr key={account.id} className="border-b border-border-base hover:bg-bg-secondary transition-colors">
                ...
                <td className="py-3 px-3">
                  <div className="flex items-center gap-1 justify-end flex-wrap">
                    ...
                    <button onClick={() => openAction('Release Invoice', account)} className="text-xs px-2 py-1 rounded bg-bg-secondary text-text-secondary hover:bg-accent-blue-light hover:text-accent-blue transition-colors">
                      Release
                    </button>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={`${modalAction} — ${selectedAccount?.client}`}>
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={() => { setModalOpen(false); }} className="flex-1 py-2.5 rounded-lg bg-accent-blue text-white text-sm font-medium hover:bg-accent-blue/90 transition-colors">
              Confirm {modalAction}
            </button>
          </div>
        </div>
                ...
                <tr key={i} className="border-b border-border-base hover:bg-bg-secondary">
                  ...
                  {['0-30', '31-60', '61-90', '90+'].map(bucket => (
                    <td key={bucket} className="py-2.5 px-4 text-right font-mono text-sm">
                      {(row as any)[bucket] > 0 ? new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format((row as any)[bucket]) : <span className="text-text-muted">—</span>}
                    </td>
                  ))}
                </tr>
              ))}
            ...
            {workOrders.map(wo => (
              <tr key={wo.id} className="border-b border-border-base hover:bg-bg-secondary">
                ...
                <td className="py-3 px-3 text-right">
                  ...
                  {wo.invoiceReady && (
                    <button className="text-xs px-3 py-1.5 rounded bg-accent-blue-light text-accent-blue hover:bg-accent-blue hover:text-white transition-colors font-medium ml-1">
                      Bill
                    </button>
                  )}
                </td>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-bg-secondary rounded-xl p-4 space-y-2 text-sm">
            ...
            <div className="bg-bg-card border border-border-base rounded-lg p-3 text-xs text-text-secondary leading-relaxed">
              <p>Hi [AP Contact],</p>
              <br />
              <p>Following up on {selectedWo?.poRef} required to process work order {selectedWo?.id}. The work has been completed and we're ready to invoice, but need the PO reference to proceed.</p>
              <br />
              <p>Could you confirm PO issuance at your earliest convenience?</p>
              <br />
